...
Person::Person(JQuad* quads[], JQuad* deadquad, std::vector<Bullet*>* bullets, std::vector<GunObject*>* guns, int team, char* name, int movementstyle)
{
	...
	mIsUsingRightAnalog = false;
	mPreviousIsUsingRightAnalog = mIsUsingRightAnalog;
...
void Person::MoveCursor(int deltaCursorX, int deltaCursorY, float dt)
{
    ...
    if (mCursorX == 0) {
        ...
        targetAngle = atan2(mCursorY, mCursorX);
    }

    const double TWO_PI = 2 * M_PI;

    double angleDifference = fmod(targetAngle - mFacingAngle + M_PI, TWO_PI);
    if (angleDifference < 0) {
        angleDifference += TWO_PI;
    }
    angleDifference -= M_PI;

    const double fixedSpeed = 0.005;

    double normalizedDifference = fabs(angleDifference) / M_PI;
    double stepScale = pow(normalizedDifference, 0.2);
    double effectiveSpeed = fixedSpeed * stepScale;
    double rotationDelta = effectiveSpeed * dt;

    if (fabs(angleDifference) < rotationDelta) {
        mFacingAngle = targetAngle;
    } else {
        if (angleDifference > 0) {
            mFacingAngle += rotationDelta;
        } else {
            mFacingAngle -= rotationDelta;
        }
    }
