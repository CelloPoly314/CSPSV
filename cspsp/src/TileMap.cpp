...
{   
    ...
    float mapX = x - SCREEN_WIDTH_2 + 0.5f;
    float mapY = y - SCREEN_HEIGHT_2 + 0.5f;

    int col = (int)(mapX / 32);
    float xoffset = -std::round(std::fmod(mapX, 32.0f) * 2.0f) / 2.0f;
    int row = (int)(mapY / 32) - 1;
    float yy = -std::round(std::fmod(mapY, 32.0f) * 2.0f) / 2.0f - 32.0f;
