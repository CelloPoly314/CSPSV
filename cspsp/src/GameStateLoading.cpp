		...
		case 0: {
			...
			for (int i=0; i<2; i++) {
				for (int j=0; j<4; j++) {
					...
					gPlayersDeadQuads[i][j] = new JQuad(playersDeadTexture,(j+i*4)*32,0,29,47);
		...
		case 1: {
			...
			for (int i=0;i<(gunsTexture->mTexHeight)/32;i++) {
				for (int j=0;j<(gunsTexture->mTexWidth)/32;j++) {
					...
					if (k == 14 || k == 20 || k == 21|| k == 22) {
						gGunGroundQuads[k] = new JQuad(gunsgroundTexture,j*32.0f,i*32.0f,31.5f,32.0f);
