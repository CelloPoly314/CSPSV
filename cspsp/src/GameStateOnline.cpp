...
void GameStateOnline::CheckInput(float dt)
{
	...
	int deltaMouseX = mEngine->GetAnalogRX();
	int deltaMouseY = mEngine->GetAnalogRY();
	...
	if (!mEngine->GetButtonClick(CTRL_CIRCLE) && !mEngine->GetButtonClick(CTRL_CROSS)) {
		if (std::abs(deltaMouseX) >= 50 || std::abs(deltaMouseY) >= 50) {
			mPlayer->mIsUsingRightAnalog = true;
			mPlayer->MoveCursor(deltaMouseX, deltaMouseY, dt);
		}
	}
	else {
		if (std::abs(deltaMouseX) < 50 && std::abs(deltaMouseY) < 50) {
		mPlayer->mIsUsingRightAnalog = false;
		}
	}

	if(mPlayer->mIsUsingRightAnalog){
		...
		if (mEngine->GetButtonState(CTRL_RTRIGGER) && !cross)
		{
				bool fire = true;
...
void GameStateOnline::Update(float dt)
{
	...
	if (mState == PLAYING) {
		...
		if (mPlayer->mIsUsingRightAnalog != mPlayer->mPreviousIsUsingRightAnalog) {
			if (mPlayer->mIsUsingRightAnalog)
				mHud->SetMessage("Vita control: on");
			else
				mHud->SetMessage("Vita control: off");
			mPlayer->mPreviousIsUsingRightAnalog = mPlayer->mIsUsingRightAnalog;
		}
	...
	if (mState != PLAYING) {
		...
		if (mState == CONNECTING) {
			...
			if (mEngine->GetButtonClick(CTRL_CROSS)) {
				...
				char directory[256];
				sprintf(directory,"ux0:data/CSPSP/maps/%s",mMapName);
