...
{
	mFrameCounter++;
	if (mFrameCounter >= mCheckInterval) {
        mFrameCounter = 0;
    }
	...
	switch (mAIState) {
		case AI_IDLE: {
			if (mFrameCounter == mCheckInterval-1) {
				mMoveTargetX = mX;
				mMoveTargetY = mY;
		...
		case AI_SEARCHING: {
			if (mFrameCounter == mCheckInterval-1) {
				...
				if (mTargetNode == NULL) {
					...
					float dy = mTarget->mY-mY;
					float distance = dx*dx + dy*dy;
					float distancef = sqrtf(distance);
					...
					mPath = mAStar->GetPath(mNode,endnode,(int)distancef*15);
		...
		case AI_ATTACKING: {
			...
			if (mFrameCounter == mCheckInterval-1) {
				...
				float dy = mTarget->mY-mY;
				float distance = dx*dx + dy*dy;
				float distancef = sqrtf(distance)/0.25f;

				if (mCanSeeEnemy) {
					mFaceTargetX = mTarget->mX+mTarget->mSpeed*cosf(mTarget->mAngle)*(distancef);
					mFaceTargetY = mTarget->mY+mTarget->mSpeed*sinf(mTarget->mAngle)*(distancef);
				}
				if (distance > 50000) {
					SetAIState(AI_SEARCHING);
					break;
